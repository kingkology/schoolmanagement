...
class Admin extends CI_Controller
{
    ...
    function student_marksheet($student_id = '') {
        ...
        $page_data['page_title'] =   get_phrase('marksheet_for') . ' ' . $student_name . ' (' . get_phrase('') . ' ' . $class_name . ')';
    ...
    function marks($exam_id = '', $class_id = '', $subject_id = '')
    {
        ...
        if ($this->input->post('operation') == 'update') {
            ...
            foreach($students as $row) {
                ...
                $data['class_mark'] = $this->input->post('class_mark_' . $row['student_id']);
                $data['final_score'] = ($this->input->post('class_mark_' . $row['student_id'])) + ($this->input->post('mark_obtained_' . $row['student_id']))  ;
                $data['comment']       = $this->input->post('comment_' . $row['student_id']);
                
                $this->db->where('mark_id', $this->input->post('mark_id_' . $row['student_id']));
                $this->db->update('mark', array('mark_obtained' => $data['mark_obtained'] , 'class_mark' => $data['class_mark'] , 'mark_total' => $data['final_score'] , 'comment' => $data['comment']));
            }
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
    ...
    function class_routine($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['day']            = $this->input->post('day');
             ...
             if($data['class_id']!="" AND $data['subject_id']!="" AND $data['time_start']!="" AND $data['time_end']!="" AND $data['time_start_min']!="" AND $data['time_end_min']!="" AND $data['day']!="")
        {
            ...
            else
            {
                 ...
                 $this->session->set_flashdata('flash_message' , "Please Fill All Fields");
           redirect(base_url() . 'index.php?admin/class_routine/', 'refresh');
            exit;
            }
